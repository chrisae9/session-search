...
First enable [primary verification](../deploy/README.md) with a complete recovery receipt and enough restore workspace. Replica availability alone is insufficient. Each client needs exact raw acknowledgements from successful archival uploads; older queue records without that mapping are reported as missing and remain in place. A successful forced archival recapture and flush can establish missing acknowledgements.
