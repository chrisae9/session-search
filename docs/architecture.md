...
```mermaid
flowchart LR
    client["Agent clients"] -->|"Search and upload"| primary["Primary service"]
    primary --> history[("Canonical history")]
    primary -.->|"Optional semantic search"| model["Embedding provider"]
    primary -.->|"Optional verified replication"| standby["Read-only search replica"]
```
...
```mermaid
flowchart LR
    history[("Canonical history and raw archives")] --> consistent["Consistent backup input"]
    consistent --> backup["Operator backup system"]
    backup --> restore["Restore and verify"]
```
